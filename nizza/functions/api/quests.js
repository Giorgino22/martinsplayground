...
async function ensure(env) {
  await env.DB.prepare('CREATE TABLE IF NOT EXISTS quests (id INTEGER PRIMARY KEY AUTOINCREMENT, category TEXT NOT NULL, title TEXT NOT NULL, amount INTEGER NOT NULL)').run();
...
async function all(env) {
  const { results } = await env.DB.prepare('SELECT id, category, title, amount FROM quests ORDER BY id').all();
  return results;
}
...
export async function onRequestPost({ env, request }) {
  ...
  await env.DB.prepare('INSERT INTO quests (category, title, amount) VALUES (?, ?, ?)').bind(category, t, a).run();
  return json({ quests: await all(env) });
}
